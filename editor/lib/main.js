...
/**Prueba de poner asi el fondo*/
var fondoCuatro = false;
...
function addBackground(canvasElement){
    ...
    if (!backgroundImage) {
        ...
        if(gridVisible){
            ...
            for(var i=0;i<rows; i++){
                for(var j=0; j<columns; j++){
                    ...
                    //big cross
                    ctx.moveTo(j * GRIDWIDTH - 20, i * GRIDWIDTH);
                    ctx.lineTo(j * GRIDWIDTH + 20, i * GRIDWIDTH);

                    ctx.moveTo(j * GRIDWIDTH, i * GRIDWIDTH - 20);
                    ctx.lineTo(j * GRIDWIDTH, i * GRIDWIDTH + 20);


                    //small dot
                    ctx.moveTo(j * GRIDWIDTH + GRIDWIDTH/2, i * GRIDWIDTH +  GRIDWIDTH/2 - 100);
                    ctx.lineTo(j * GRIDWIDTH + GRIDWIDTH/2, i * GRIDWIDTH +  GRIDWIDTH/2 + 100);
        ...
        // probando esta forma de meter el fondo
        if(fondoCuatro){
  
            /**Para poder pintar el nombre del usuario*/
            var usuario = document.getElementById('usuario').innerHTML;

            //backgroundImage = new Image();
            //backgroundImage.onload = function(e){
            //ctx.drawImage(this, 0, 0); //XXX

            ctx.fillStyle="#000000";
            ctx.font="15px Georgia";
            ctx.fillText('Versión no.',614,30);
            ctx.fillText('Fecha',614,55);
            ctx.fillText('Autor',614,77);
            ctx.fillText(usuario,660,77);

            //}
            //backgroundImage.src = './assets/images/recuadro_3.png';
        }
...
{   
    ...
    var imageObj = new Image();
                ...
                context.fillText('Versión no.',790,50);
                context.fillText('Fecha',790,75);
                context.fillText('Autor',790,100);
                context.fillText(usuario,845,100);
                };
                imageObj.src = "./assets/images/recuadro_1_2.png";

    var undo = new CanvasChangeSizeCommand(1000, 800);
    undo.execute();
    History.addUndo(undo);

}//end draw nivel1

/*the function for nivel2 */
function drawNivel2()
{   
    var d_canvas = document.getElementById('a');
    var context = d_canvas.getContext('2d');
...
{   
    ...
    var imageObj = new Image();
                ...
                context.fillStyle="#000000";
                context.font="15px Georgia";
                context.fillText('Versión no.',614,30);
                context.fillText('Fecha',614,55);
                context.fillText('Autor',614,77);
                context.fillText(usuario,660,77);
                };
                imageObj.src = "./assets/images/recuadro_3.png";

    var undo = new CanvasChangeSizeCommand(800, 1000);
    undo.execute();
    History.addUndo(undo);

}//end draw nivel3

/*the function for nivel4 */
function drawNivel4()
{   
    var d_canvas = document.getElementById('a');
    var context = d_canvas.getContext('2d');
